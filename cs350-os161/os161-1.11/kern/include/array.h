...
#define DECLARRAY_BYTYPE(ARRAY, T) \
	...
	int ARRAY##_getnum(struct ARRAY *a);		        \
	void *ARRAY##_getguy(struct ARRAY *a, int index);	\
	void ARRAY##_setguy(struct ARRAY *a, int index, void *val); \
	int ARRAY##_preallocate(struct ARRAY *a, int num);	\
	int ARRAY##_add(struct ARRAY *a, void *guy); \
        int ARRAY##_setsize(struct ARRAY *a, int nguys) \
	void ARRAY##_remove(struct ARRAY *a, int index)
